...
describe("Private DEX", function () {
  ...
  it("Encrypted DEX Pool", async function () {
    ...
    const tx0 = await token0.mint(2_000_000_000n);
    await tx0.wait();
    const instances0 = await createInstances(token0Address, ethers, this.signers);
    const tx1 = await token1.mint(2_000_000_000n);
    await tx1.wait();
    ...
    console.log("Bob's token0 balance : ", await getPrivateBalanceERC20(token0Address, "bob"));
    console.log("Bob's token1 balance : ", await getPrivateBalanceERC20(token1Address, "bob"));
    console.log("Carol's token0 balance : ", await getPrivateBalanceERC20(token0Address, "carol"));
    console.log("Carol's token1 balance : ", await getPrivateBalanceERC20(token1Address, "carol"));

    const tx6 = await token0["transfer(address,bytes)"](
      this.signers.dave.address,
      instances0.alice.encrypt32(1_000_000),
    );
    await tx6.wait();
    const tx6bis = await token1["transfer(address,bytes)"](
      this.signers.dave.address,
      instances1.alice.encrypt32(0), // to obfuscate the direction of the swap later, needed to initialize dave's token1 balance
    );
    await tx6bis.wait();

    const tx7 = await token1["transfer(address,bytes)"](
      this.signers.eve.address,
      instances1.alice.encrypt32(1_000_000),
    );
    await tx7.wait();
    const tx7bis = await token0["transfer(address,bytes)"](
      this.signers.eve.address,
      instances0.alice.encrypt32(0), // to obfuscate the direction of the swap later, needed to initialize eve's token0 balance
    );
    await tx7bis.wait();

    console.log("Initial balances of traders (Dave and Eve) : ");
    console.log("Dave's token0 balance : ", await getPrivateBalanceERC20(token0Address, "dave"));
    console.log("Dave's token1 balance : ", await getPrivateBalanceERC20(token1Address, "dave"));
    console.log("Eve's token0 balance : ", await getPrivateBalanceERC20(token0Address, "eve"));
    console.log("Eve's token1 balance : ", await getPrivateBalanceERC20(token1Address, "eve"), "\n");
    ...
    const tx9 = await token0
      ...
      ["approve(address,bytes)"](pairAddress, instances0.bob.encrypt32(100_000_000));
    await tx9.wait();
    const tx10 = await token1
      .connect(this.signers.bob)
      ["approve(address,bytes)"](pairAddress, instances1.bob.encrypt32(200_000_000));
    await tx10.wait();

    const tx11 = await pair
      .connect(this.signers.bob)
      .addLiquidity(
        instancesPair.bob.encrypt32(100_000_000),
        instancesPair.bob.encrypt32(200_000_000),
        this.signers.bob.address,
        0n,
      );
    await tx11.wait();
    ...
    const tx12 = await token0
      ...
      ["approve(address,bytes)"](pairAddress, instances0.carol.encrypt32(200_000_000));
    await tx12.wait();
    const tx13 = await token1
      .connect(this.signers.carol)
      ["approve(address,bytes)"](pairAddress, instances0.carol.encrypt32(400_000_000));
    await tx13.wait();
    const tx14 = await pair
      .connect(this.signers.carol)
      .addLiquidity(
        instancesPair.carol.encrypt32(200_000_000),
        instancesPair.carol.encrypt32(400_000_000),
        this.signers.carol.address,
        0n,
      );
    await tx14.wait();
    ...
    console.log("Bob now owns a private balance of ", balance, " liquidity tokens");
    ...
    const tx19 = await token1
      .connect(this.signers.dave)
      ["approve(address,bytes)"](pairAddress, instances1.bob.encrypt32(100_000_000));
    await tx19.wait();
    const tx20 = await pair
      .connect(this.signers.dave)
      .swapTokens(
        instancesPair.dave.encrypt32(1_000_000),
        instancesPair.dave.encrypt32(0),
        this.signers.dave.address,
        1n,
      );
    await tx20.wait();
    ...
    const tx23 = await pair
      .connect(this.signers.eve)
      .swapTokens(instancesPair.eve.encrypt32(0), instancesPair.eve.encrypt32(1_000_000), this.signers.eve.address, 1n);
    await tx23.wait();
    ...
    const tx27 = await pair
      .connect(this.signers.bob)
      .removeLiquidity(instancesPair.bob.encrypt32(149999900 / 2), this.signers.bob.address, 2n);
    await tx27.wait();
    console.log("Bob submitted a removeLiquidity order at tradingEpoch ", await pair.currentTradingEpoch());

    const tx28 = await pair
      .connect(this.signers.carol)
      .removeLiquidity(instancesPair.carol.encrypt32(299999900 / 2), this.signers.carol.address, 2n);
    await tx28.wait();
